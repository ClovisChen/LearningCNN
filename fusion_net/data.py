...
class Data(object):
    ...
    def read_image(self, image_path):
        ...
        # if the dataset is cityscapes, we crop(修剪) the last fifth to remove the car hood(引擎盖)
    ...
    # ---------------------------------------seg------------------------------------------------------
    def create_filewrite_handler(self):
        """
        ...
        """
        target_dir = os.path.dirname(self.logging_file)
        if not os.path.exists(self.target_dir):
            os.makedirs(self.target_dir)
        filewriter = logging.FileHandler(self.logging_file, mode='w')
    ...
    def initialize_training_folder(self, files_dir="model_files", logging=True):
        """
        ...
        hypes : dict
            Hyperparameters
        """
        output_dir = '/home/data/code/learningReloc/fusion_net/RUNS'
        target_dir = os.path.join(output_dir, files_dir)
        ...
        # Creating an additional logging saving the console outputs into the training folder
        if logging:
            self.logging_file = os.path.join(output_dir, "output.log")
            self.create_filewrite_handler(self.logging_file)
