...
def set_dirs(hypes, hypes_fname):
    ...
    if 'data_dir' not in hypes['dirs']:
        if 'TV_DIR_DATA' in os.environ:
            ...
            data_dir = os.path.join(base_path, '../RUNS')
...
def load_modules_from_logdir(logdir, dirname="model", postfix=""):
...
def load_hypes_from_logdir(logdir, subdir="model", base_path=None, json_file="hypes.json"):
    """Load hypes from the logdir.
    ...
    hypes_fname = os.path.join(model_dir, json_file)
    ...
    hypes['dirs']['data_dir'] = hypes['dirs']['base_path'] + data_dir
