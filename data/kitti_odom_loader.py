...
class kitti_odom_loader(object):
    ...
    def collect_test_frames(self):
        ...
        for seq in self.test_seqs:
            ...
            img_dir = os.path.join(seq_dir, image_folder_name)
    ...
    def is_valid_sample(self, frames, tgt_idx):
        N = len(frames)
        tgt_drive, _ = frames[tgt_idx-1].split(' ')
    ...
    def load_image(self, drive, frame_id):
        img_file = os.path.join(self.dataset_dir, 'sequences', '%s/%s/%s.png' % (drive, image_folder_name, frame_id))
